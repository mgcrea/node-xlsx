...
function xlsx(file, options) {
  ...
  var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, style, borders, border, borderIndex, fonts, font, fontIndex,
    ...
    alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', coreProps;
  ...
  function convertDate(input) {
    if(typeof input === 'object') {
      return ((input - new Date(1900, 0, 0)) / 86400000) + 1;
    }
    return new Date(new Date(1900, 0, 0) + (input - 1) * 86400000);
  }
  ...
  if (typeof file === 'string') {
    ...
    coreProps = zip.file('docProps/core.xml');
    if (coreProps) {
      s = coreProps.asText();
    ...
    // Get workbook info from "xl/workbook.xml" - Worksheet names exist in other places,
    // but "activeTab" attribute must be gathered from this file anyway
